...
function PromptInput({
  ...
  const onSubmit = useCallback(async (inputParam: string, isSubmittingSlashCommand = false) => {
    ...
    if (suggestionsState.suggestions.length > 0 && !isSubmittingSlashCommand && !hasDirectorySuggestions) {
      if (suggestionsState.selectedSuggestion !== -1) {
        // A suggestion is explicitly selected — user should Tab to accept, not Enter to submit
        logForDebugging(`[onSubmit] early return: suggestion selected (index=${suggestionsState.selectedSuggestion})`);
        return;
      }
      // No suggestion explicitly selected — dismiss suggestions and proceed with submission
      setSuggestionsState({ suggestions: [], selectedSuggestion: -1, commandArgumentHint: undefined });
    }
