...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // Personal fork: auto-bootstrap onboarding and trust without dialogs
  ...
  /* Original dialog code below — kept for reference but unreachable */
  ...
  // Check for custom API key — auto-approve in this personal fork (no dialog needed)
  if (process.env.ANTHROPIC_API_KEY && !isRunningOnHomespace()) {
    const customApiKeyTruncated = normalizeApiKeyForConfig(process.env.ANTHROPIC_API_KEY);
    const keyStatus = getCustomApiKeyStatus(customApiKeyTruncated);
    if (keyStatus === 'new') {
      // Auto-approve without showing dialog for personal use
      saveGlobalConfig(current => ({
        ...current,
        customApiKeyResponses: {
          ...current.customApiKeyResponses,
