...
// Note: require() of async/ESM modules is unsupported in Bun runtime — use dynamic import with fallback
const _fallbackMessageSelector = {
  selectableUserMessagesFilter: (_msg: unknown) => true,
  messagesAfterAreOnlySynthetic: (_messages: unknown[], _idx: number) => false,
  MessageSelector: null,
}
const _messageSelectorCache: { mod: typeof import('src/components/MessageSelector.js') | null } = { mod: null }
// Kick off async load immediately — will be ready before first query
import('src/components/MessageSelector.js').then(m => {
  _messageSelectorCache.mod = m as typeof import('src/components/MessageSelector.js')
}).catch(() => {})
const messageSelector = (): typeof import('src/components/MessageSelector.js') => {
  return (_messageSelectorCache.mod ?? _fallbackMessageSelector) as typeof import('src/components/MessageSelector.js')
}
